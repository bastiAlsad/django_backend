...
from .models import Laxout_User, Laxout_Exercise
...
def delete_user(request, id=None):
    if id != None:
        object_to_delte = Laxout_User.objects.get(id=id)
...
def edit_user(request, id=None):
    users = Laxout_User.objects.get(id=id)
    return render(
        request,
...
def add_exercises(request, id=None, first=0, second=0):
    ...
    if request.method == "POST":
        ...
        print(new_dauer)
        user_instance = Laxout_User.objects.get(id=id)
...
def edit_user_workout(request, id=None,):
    if request.method == "POST":
        ...
        print(new_dauer)
        user_instance = Laxout_User.objects.get(id=user_id)
...
# api logic
@csrf_exempt
def get_user_uid(request, access_code = None):
    if request.method == "GET":
       
           user_instance = Laxout_User.objects.get(access_code = access_code)
           response = [{
